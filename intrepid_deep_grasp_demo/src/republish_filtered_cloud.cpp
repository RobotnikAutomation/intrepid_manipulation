...
pcl::PointCloud<pcl::PointXYZRGB> *cloudXYZ_one = new pcl::PointCloud<pcl::PointXYZRGB>;
pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloudXYZPtr1(cloudXYZ_one);

pcl::PointCloud<pcl::PointXYZRGB> *cloudXYZ_two = new pcl::PointCloud<pcl::PointXYZRGB>;
pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloudXYZPtr2(cloudXYZ_two);

pcl::PointCloud<pcl::PointXYZRGB> *cloud_icp = new pcl::PointCloud<pcl::PointXYZRGB>;
pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloudPtrFinal(cloud_icp);
...
{
    // Initialize ROS
    ros::init(argc, argv, "republish_filtered_cloud");
    ros::NodeHandle n;
    ...
    octomap_pub = n.advertise<sensor_msgs::PointCloud2>("/octomap_pcl", buffer_size);
    ...
    while(ros::ok())
    {
        ...
        seg1.setInputCloud (cloudPtrFinal);
        ...
        extract.setInputCloud (cloudPtrFinal);
        extract.setIndices (inliers);
        extract.setNegative (false);
        extract.filter (*xyzCloudPtrRansacFiltered);

        octomap_pub.publish(*xyz_cloud_ransac_filtered); // Publish
        r.sleep();
        ros::spinOnce();
    }
    return 0;
